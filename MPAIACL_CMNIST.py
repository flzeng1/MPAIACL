...
from common_utils import info_nce_DA, parameter_visualization, contrast_loss
...
def train():
    ...
    for step, data in enumerate(train_loader):
        ...
        loss_cl_con = cl_criterion_DA(h_graph_cau, h_graph_adv)
...
def evaluate():
    ...
    if valid_cau > results['highest_valid'] and epoch > args.test_epoch:
        ...
        save_model = './save/MPAIACL_params_' + args.dataset + '_' + args.domain + '.pth'
...
dataset, _ = GOODCMNIST.load(args.path, domain=args.domain, shift=args.shift, generate=False)
...
for repetition in range(args.repetition):
    ...
    for epoch in pbar:
        ...
        pbar.set_postfix(OrderedDict(
            [('best_epoch', f"{results['update_epoch']:.4f}"), ('best_val_acc', f"{results['highest_valid']:.4f}"),
