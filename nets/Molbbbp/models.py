...
from nets.Molbbbp.conv_syn import GNNSynEncoder, GraphSynMasker
from nets.Molbbbp.conv_mol_new import GINMolHeadEncoder, GraphMolMasker, GNNMolTailEncoder
from nets.Molbbbp.utils import get_h_causaler, get_h_DA
...
class CausalAdvGNNSyn(torch.nn.Module):
    ...
    def forward_attack(self, data):
        ...
        adv_node_reg = self.reg_mask_loss(node_adv_num, node_env_num, self.adv_gamma_node,
                                          self.attacker.non_zero_node_ratio)
        adv_edge_reg = self.reg_mask_loss(edge_adv_num, edge_env_num, self.adv_gamma_edge,
                                          self.attacker.non_zero_edge_ratio)
...
class CausalAdvGNNMol(torch.nn.Module):
    ...
    def forward_attack(self, data, model):
        x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        x_encode = self.graph_front(x, edge_index, edge_attr, batch)
        ...
        loss_dis = loss_dis + self.wasserstein_distance(h_graph_adv, h_graph_ori)
        loss_dis = loss_dis + self.wasserstein_distance(h_graph_adv, h_graph_cau)
        loss_dis = loss_dis + self.wasserstein_distance(h_graph_adv, h_graph_com)
