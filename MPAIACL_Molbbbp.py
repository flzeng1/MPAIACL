...
from nets.Molbbbp.utils import get_h_causaler, get_h_attack, get_h_DA, get_dataloader, eval
from common_utils import info_nce_DA, contrast_loss, parameter_visualization

from args import parse_args
from nets.Molbbbp.models import CausalAdvGNNMol
from common_utils import set_random_seed, get_OPT, get_schedule,   \
...
def train():
    ...
    for step, data in enumerate(train_loader):
        ...
        loss_cl_con = cl_criterion_DA(h_graph_cau, h_graph_com, h_graph_adv)
        ...
        loss_causaler = loss_cl_con * args.con_reg + loss_cau_truth + loss_com_truth
    ...
    writer.add_scalar("epoch_loss_attacker", epoch_loss_attacker, epoch)



def evaluate():
    global patient, is_early_stop
...
file_name = f"{args.dataset}_{args.repetition}_{current_time}"
file_path = os.path.join(args.verbose_path, args.dataset, args.domain, file_name)

dataset = PygGraphPropPredDataset(name=args.dataset, root=args.path)
...
for repetition in range(args.repetition):
    ...
    opt_attacker, opt_causaler = get_OPT(model, args, args.l2reg)
    scheduler_attacker, scheduler_causaler = get_schedule(opt_attacker, opt_causaler, args)


    cl_criterion_DA = contrast_loss
    ...
    for epoch in pbar:
        ...
        pbar.set_postfix(OrderedDict([('best_epoch', f"{results['update_epoch']:.4f}"), ('best_val_acc', f"{results['highest_valid']:.4f}"),
                                  ('best_test_acc', f"{results['update_test']:.4f}")]))
        if is_early_stop:
            break
...
print_log(args, final_test_acc, total_time, model, args_list)
